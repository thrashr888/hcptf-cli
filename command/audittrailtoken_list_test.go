...
func TestAuditTrailTokenListRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &AuditTrailTokenListCommand{
		Meta: newTestMeta(ui),
	}
...
func TestAuditTrailTokenListHelp(t *testing.T) {
	cmd := &AuditTrailTokenListCommand{}
	...
	if !strings.Contains(help, "hcptf audittrailtoken list") {
...
func TestAuditTrailTokenListSynopsis(t *testing.T) {
	cmd := &AuditTrailTokenListCommand{}
	...
	if synopsis != "Show the audit trail token for an organization" {
		t.Errorf("expected 'Show the audit trail token for an organization', got %q", synopsis)
	}
}

func TestAuditTrailTokenListFlagParsing(t *testing.T) {
	tests := []struct {
		name           string
		args           []string
		expectedOrg    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("audittrailtoken list")
