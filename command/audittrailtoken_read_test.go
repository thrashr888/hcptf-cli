...
func TestAuditTrailTokenReadRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &AuditTrailTokenReadCommand{
		Meta: newTestMeta(ui),
	}
...
func TestAuditTrailTokenReadHelp(t *testing.T) {
	cmd := &AuditTrailTokenReadCommand{}
	...
	if !strings.Contains(help, "hcptf audittrailtoken read") {
...
func TestAuditTrailTokenReadSynopsis(t *testing.T) {
	cmd := &AuditTrailTokenReadCommand{}
	...
	if synopsis != "Read audit trail token details" {
		t.Errorf("expected 'Read audit trail token details', got %q", synopsis)
	}
}

func TestAuditTrailTokenReadFlagParsing(t *testing.T) {
	tests := []struct {
		name           string
		args           []string
		expectedOrg    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("audittrailtoken read")
