...
func TestOrganizationTokenDeleteRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &OrganizationTokenDeleteCommand{
		Meta: newTestMeta(ui),
	}
...
func TestOrganizationTokenDeleteHelp(t *testing.T) {
	cmd := &OrganizationTokenDeleteCommand{}
	...
	if !strings.Contains(help, "hcptf organizationtoken delete") {
	...
	if !strings.Contains(help, "permanently") || !strings.Contains(help, "invalidate") {
		t.Error("Help should warn about permanent deletion")
	}
}

func TestOrganizationTokenDeleteSynopsis(t *testing.T) {
	cmd := &OrganizationTokenDeleteCommand{}
	...
	if synopsis != "Delete an organization token" {
		t.Errorf("expected 'Delete an organization token', got %q", synopsis)
	}
}

func TestOrganizationTokenDeleteFlagParsing(t *testing.T) {
	tests := []struct {
		name          string
		args          []string
		expectedOrg   string
		...
		},
		{
			...
			args:          []string{"-organization=prod-org", "-force"},
			expectedOrg:   "prod-org",
			expectedForce: true,
		},
		{
			name:          "org alias with force=true",
			args:          []string{"-org=dev-org", "-force=true"},
			expectedOrg:   "dev-org",
			expectedForce: true,
		},
		{
			name:          "organization with force=false",
			args:          []string{"-organization=keep-org", "-force=false"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("organizationtoken delete")
