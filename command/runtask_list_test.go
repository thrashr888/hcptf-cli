...
func TestRunTaskListRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &RunTaskListCommand{
		Meta: newTestMeta(ui),
	}
...
func TestRunTaskListHelp(t *testing.T) {
	cmd := &RunTaskListCommand{}
	...
	if !strings.Contains(help, "hcptf runtask list") {
...
func TestRunTaskListSynopsis(t *testing.T) {
	cmd := &RunTaskListCommand{}
	...
	if synopsis != "List run tasks in an organization" {
		t.Errorf("expected 'List run tasks in an organization', got %q", synopsis)
	}
}

func TestRunTaskListFlagParsing(t *testing.T) {
	tests := []struct {
		name        string
		args        []string
		expectedOrg string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("runtask list")
