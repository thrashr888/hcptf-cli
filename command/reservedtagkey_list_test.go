...
func TestReservedTagKeyListRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &ReservedTagKeyListCommand{
		Meta: newTestMeta(ui),
	}
...
func TestReservedTagKeyListHelp(t *testing.T) {
	cmd := &ReservedTagKeyListCommand{}
	...
	if !strings.Contains(help, "hcptf reservedtagkey list") {
	...
	if !strings.Contains(help, "List reserved tag keys") {
		t.Error("Help should describe listing reserved tag keys")
	}
}

func TestReservedTagKeyListSynopsis(t *testing.T) {
	cmd := &ReservedTagKeyListCommand{}
	...
	if synopsis != "List reserved tag keys" {
		t.Errorf("expected 'List reserved tag keys', got %q", synopsis)
	}
}

func TestReservedTagKeyListFlagParsing(t *testing.T) {
	tests := []struct {
		name        string
		args        []string
		expectedOrg string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("reservedtagkey list")
