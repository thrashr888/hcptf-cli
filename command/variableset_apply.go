...
func (c *VariableSetApplyCommand) Run(args []string) int {
	...
	if c.workspaces == "" && c.projects == "" {
		c.Ui.Error("Error: either -workspaces or -projects flag is required")
		c.Ui.Error(c.Help())
		return 1
	}
...
Example:
  ...
  hcptf variableset apply -id=varset-12345 -workspaces=ws-abc123 -projects=prj-abc123
`
...
func (c *VariableSetApplyCommand) Synopsis() string {
	return "Apply a variable set to workspaces or projects"
}
