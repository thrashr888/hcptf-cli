...
func TestPolicySetAddPolicyRequiresPolicySetID(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &PolicySetAddPolicyCommand{
		Meta: newTestMeta(ui),
	}
...
func TestPolicySetAddPolicyRequiresPolicyID(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &PolicySetAddPolicyCommand{
		Meta: newTestMeta(ui),
	}
...
func TestPolicySetAddPolicyRequiresAllFlags(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &PolicySetAddPolicyCommand{
		Meta: newTestMeta(ui),
	}
...
func TestPolicySetAddPolicyHelp(t *testing.T) {
	cmd := &PolicySetAddPolicyCommand{}
	...
	if !strings.Contains(help, "hcptf policyset add-policy") {
...
func TestPolicySetAddPolicySynopsis(t *testing.T) {
	cmd := &PolicySetAddPolicyCommand{}
	...
	if synopsis != "Add a policy to a policy set" {
		t.Errorf("expected 'Add a policy to a policy set', got %q", synopsis)
	}
}

func TestPolicySetAddPolicyFlagParsing(t *testing.T) {
	tests := []struct {
		name               string
		args               []string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("policyset add-policy")
			flags.StringVar(&cmd.policySetID, "policyset-id", "", "Policy set ID (required)")
			flags.StringVar(&cmd.policyID, "policy-id", "", "Policy ID to add (required)")
