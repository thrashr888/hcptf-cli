...
func TestVariableSetListRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &VariableSetListCommand{
		Meta: newTestMeta(ui),
	}
...
func TestVariableSetListHelp(t *testing.T) {
	cmd := &VariableSetListCommand{}
	...
	if !strings.Contains(help, "hcptf variableset list") {
...
func TestVariableSetListSynopsis(t *testing.T) {
	cmd := &VariableSetListCommand{}
	...
	if synopsis != "List variable sets in an organization" {
		t.Errorf("expected 'List variable sets in an organization', got %q", synopsis)
	}
}

func TestVariableSetListFlagParsing(t *testing.T) {
	tests := []struct {
		name        string
		args        []string
		expectedOrg string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("variableset list")
