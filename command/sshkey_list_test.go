...
func TestSSHKeyListRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &SSHKeyListCommand{
		Meta: newTestMeta(ui),
	}
...
func TestSSHKeyListHelp(t *testing.T) {
	cmd := &SSHKeyListCommand{}
	...
	if !strings.Contains(help, "hcptf sshkey list") {
...
func TestSSHKeyListSynopsis(t *testing.T) {
	cmd := &SSHKeyListCommand{}
	...
	if synopsis != "List SSH keys for an organization" {
		t.Errorf("expected 'List SSH keys for an organization', got %q", synopsis)
	}
}

func TestSSHKeyListFlagParsing(t *testing.T) {
	tests := []struct {
		name        string
		args        []string
		expectedOrg string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("sshkey list")
