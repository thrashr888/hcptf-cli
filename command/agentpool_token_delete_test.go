...
func TestAgentPoolTokenDeleteRequiresID(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &AgentPoolTokenDeleteCommand{
		Meta: newTestMeta(ui),
	}
...
func TestAgentPoolTokenDeleteHelp(t *testing.T) {
	cmd := &AgentPoolTokenDeleteCommand{}
	...
	if !strings.Contains(help, "hcptf agentpool token-delete") {
...
func TestAgentPoolTokenDeleteSynopsis(t *testing.T) {
	cmd := &AgentPoolTokenDeleteCommand{}
	...
	if synopsis != "Delete an agent token" {
		t.Errorf("expected 'Delete an agent token', got %q", synopsis)
	}
}

func TestAgentPoolTokenDeleteFlagParsing(t *testing.T) {
	tests := []struct {
		name          string
		args          []string
		expectedID    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("agentpool token-delete")
			flags.StringVar(&cmd.id, "id", "", "Agent token ID (required)")
