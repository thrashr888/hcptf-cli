...
func TestPolicySetListRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &PolicySetListCommand{
		Meta: newTestMeta(ui),
	}
...
func TestPolicySetListHelp(t *testing.T) {
	cmd := &PolicySetListCommand{}
	...
	if !strings.Contains(help, "hcptf policyset list") {
...
func TestPolicySetListSynopsis(t *testing.T) {
	cmd := &PolicySetListCommand{}
	...
	if synopsis != "List policy sets" {
		t.Errorf("expected 'List policy sets', got %q", synopsis)
	}
}

func TestPolicySetListFlagParsing(t *testing.T) {
	tests := []struct {
		name        string
		args        []string
		expectedOrg string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("policyset list")
