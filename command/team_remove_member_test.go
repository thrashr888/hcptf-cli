...
func TestTeamRemoveMemberRequiresOrganization(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &TeamRemoveMemberCommand{
		Meta: newTestMeta(ui),
	}
...
func TestTeamRemoveMemberRequiresTeam(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &TeamRemoveMemberCommand{
		Meta: newTestMeta(ui),
	}
...
func TestTeamRemoveMemberRequiresUsername(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &TeamRemoveMemberCommand{
		Meta: newTestMeta(ui),
	}
...
func TestTeamRemoveMemberRequiresAllFlags(t *testing.T) {
	ui := cli.NewMockUi()
	cmd := &TeamRemoveMemberCommand{
		Meta: newTestMeta(ui),
	}
...
func TestTeamRemoveMemberHelp(t *testing.T) {
	cmd := &TeamRemoveMemberCommand{}
	...
	if !strings.Contains(help, "hcptf team remove-member") {
...
func TestTeamRemoveMemberSynopsis(t *testing.T) {
	cmd := &TeamRemoveMemberCommand{}
	...
	if synopsis != "Remove a member from a team" {
		t.Errorf("expected 'Remove a member from a team', got %q", synopsis)
	}
}

func TestTeamRemoveMemberFlagParsing(t *testing.T) {
	tests := []struct {
		name             string
		args             []string
		expectedOrg      string
		...
		{
			name:             "removing user from ops team",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			flags := cmd.Meta.FlagSet("team remove-member")
			...
			flags.StringVar(&cmd.username, "username", "", "Username to remove (required)")
